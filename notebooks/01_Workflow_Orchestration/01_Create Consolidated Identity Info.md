# Step 1: Create Consolidated Identity Information
...
- **Key Fields**: 
  - `request_kv._server_email` - Hashed email addresses
  - `request_kv._server_ifa` - Identifier for Advertising (mobile device IDs)
...
## Step 1.1: Load Raw Impression Logs
...
1. **`request_kv`** - JSON object containing identity signals:
   - `_server_email` - Hashed email address (our primary identifier)
   - `_server_ifa` - Identifier for Advertising (mobile device ID)
...
- **IFA**: Links to mobile app usage and cross-device behavior  
...
## Step 1.3: Create Identity Aggregations
...
|-------------|-------------|
| `_server_email` | Hashed email address (primary key) |
| `ip_address` | Associated IP address |
| `_server_ifa` | Associated mobile device identifier |
...
1. **`02a_Create Email IFA Paired Table`** - Links emails with their strongest IFA
2. **`02b_Create Email IP Paired Table`** - Links emails with their strongest IP
